...
function App() {
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-background">
        <div className="text-center">
          <div className="w-8 h-8 border-2 border-maersk-blue border-t-transparent rounded-full animate-spin mx-auto mb-3"></div>
  ...
  const userInitials = `${user.first_name?.[0] || ''}${user.last_name?.[0] || ''}`.toUpperCase() || user.email[0].toUpperCase();
  const displayName = user.first_name ? `${user.first_name[0]}. ${user.last_name}` : user.email.split('@')[0];
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              {!isSidebarCollapsed && (
                ...
                >
                  <div className="font-bold text-white uppercase tracking-wider whitespace-nowrap">{displayName}</div>
